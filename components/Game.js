...
export default Game = ({randomNumbersCount, initialSeconds}) => {
    ...
    // const randomNumbers = Array.from({lenght: randomNumbersCount}).map(() => 1 + Math.floor(10 * Math.random()));
        ...
        <View>    
            ...
            <View style={styles.randomContainer}>
                {randomNumbers.map((randomNumber, index) => (
                    <RandomNumber key={index} id={index} number={randomNumber} isSelected={isNumberSelected(index) || gameStatus !== 'PLAYING'}
